...
\hypersetup{
  ...
  citecolor=blue,
  urlcolor=blue,
  pdftitle={Temperature-Calibrated Reservoirs: Safe Operating Regions and Test-Time Path Indicators},
  pdfauthor={Ildefons Magrans de Abril},
  pdfsubject={Temperature-controlled reservoir computing, safe operating regions, and held-out structural-shift audit},
  pdfkeywords={reservoir computing, echo state networks, temperature softmax, safe operating regions, distribution shift, sparse aggregation}
}
\setlength{\parskip}{0.35em}
\setlength{\parindent}{0em}

\title{Temperature-Calibrated Reservoirs: Safe Operating Regions and Test-Time Path Indicators}

\author{Ildefons Magrans de Abril\\
Universitat Polit\`ecnica de Catalunya - BarcelonaTech (UPC), Barcelona, Spain\\
\texttt{ildefons.magrans@upc.edu}}
...
\begin{abstract}
Reservoir computing relies on fixed recurrent dynamics, yet its standard hyperparameters mainly control recurrent scale, update timescale, hard sparsity, or reservoir size. We introduce \emph{allocation temperature} as a distinct structural configuration axis. For each reservoir unit, it continuously sharpens or flattens the distribution of recurrent weight mass across incoming connections, while a separate normalization keeps the bulk recurrent scale comparable. Unlike hard sparsity, this control does not decide which allowed edges exist. We show that increasing allocation temperature monotonically increases row entropy and its effective support, defined as the entropy-equivalent number of equally weighted recurrent inputs.

Validation data select a default temperature and a connected region of nearby temperatures with similar validation performance. At test time, two label-free path-sensitivity indicators measure how much predictions change across this validation-safe region. Prediction dispersion summarizes disagreement across all safe temperatures, and endpoint spread compares the predictions at the low- and high-temperature ends of the region. These quantities indicate whether the model output is sensitive to the structural choice, but they do not identify which temperature will have the lowest target error.

Across controlled time-series tasks, standard reservoir benchmarks, and two public sensor datasets, allocation temperature produces a structural signature that differs from gain, leak rate, and hard sparsity. Validation-safe regions contain near-optimal test temperatures more often than same-width control intervals. This enrichment remains present with a denser 25-point temperature grid and with reservoirs of size $N=150$, where the near-optimal containment rate is $2.16$--$2.86\times$ the rate of matched control intervals. Within the one-dimensional safe region, choosing the best safe temperature reduces the validation-selected default NRMSE by $0.18\%$--$1.91\%$ in the reference robustness panel, so the direct performance gain is modest. The path-sensitivity indicators remain positively associated with the improvement available inside the safe region, with correlations of $0.41$--$0.58$, although fixed binary thresholds are less stable. In the retained historical real-data summaries, Appliances Energy shows a null safe-region-versus-control comparison, whereas UCI Air Quality gives a positive result; the current public-data reconstruction notebooks recover the structural path but not these exact operational point estimates. A final distribution-shift study broadens the candidate set to an $11\times11$ temperature--leak grid. An upper-bound five-model ensemble optimized retrospectively with held-out target labels reduces NRMSE by $25.3\%$--$39.0\%$ relative to the best fixed structure chosen before deployment. A sparse ensemble fitted only on pre-deployment conditions recovers smaller gains of $12.0\%$--$19.9\%$ without held-out target labels, but degrades under block missingness. Overall, allocation temperature adds an interpretable structural dimension to reservoir design and exposes when recurrent allocation matters, while the experiments also delineate the limits of label-free test-time selection.
\end{abstract}

\noindent\textbf{Keywords:} reservoir computing; echo state networks; recurrent neural networks; structural regularization; distribution shift; sparse aggregation; safe operating regions; temperature softmax.
...
\begin{enumerate}[leftmargin=*]
  \item A new reservoir configuration axis, allocation temperature, that continuously controls recurrent weight concentration while recurrent scale is normalized separately.
  \item A theoretical characterization of this axis through row entropy and effective support, including monotonic increase of both quantities with temperature.
  \item A validation procedure that selects a default temperature and a connected validation-safe region $\mathcal{T}_{\mathrm{safe}}$.
  \item Two label-free test-time path-sensitivity indicators that quantify prediction variation across $\mathcal{T}_{\mathrm{safe}}$.
  \item Controlled tests showing that validation-safe regions transfer to held-out data and outperform same-width control intervals.
  \item Robustness experiments showing that safe-region enrichment and the continuous indicator--improvement relationship persist with a denser temperature grid and a larger reservoir.
  \item Direct comparisons with gain, leak rate, and hard sparsity, plus two public sensor datasets, showing that allocation temperature is structurally distinct but not universally the best operational control.
  \item A final distribution-shift study that compares three practically different quantities: the improvement available when target labels are used retrospectively, the performance of a sparse ensemble fitted only on pre-deployment conditions, and the ability of unlabeled target statistics to predict low-error candidates.
\end{enumerate}

The paper separates these contributions deliberately. Allocation temperature is the main structural contribution. The safe region and path-sensitivity indicators characterize how that new axis behaves when calibrated from validation data. The final distribution-shift study then measures how much broader structural opportunity exists and how difficult it is to exploit that opportunity without target labels.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{figures/fig1_conceptual_path.png}
...
Appliances Energy & 50 & 13 & temperature, gain, leak rate, sparsity & 2500/1000/1000 & Sec.~\ref{sec:real_world} \\
UCI Air Quality & 50 & 13 & temperature, gain, leak rate, sparsity & 1500/600/600 & Sec.~\ref{sec:real_world} \\
...
\midrule
Does the safe region transfer? & Best safe temperature versus the validation-selected default & Mean gain $0.00470$ [$0.00357$, $0.00579$] \\
Is the region better than an arbitrary interval? & Validation-derived region versus same-width control intervals & Mean interval-level gain advantage $0.02739$ [$0.01870$, $0.03710$] \\
Is movement safer inside the region? & Random movement inside versus random movement outside & Mean movement-gain advantage $0.03209$ [$0.02342$, $0.04143$] \\
Do prediction differences indicate opportunity? & High- versus low-indicator test windows & Mean gain difference $0.01697$ [$0.01228$, $0.02197$] \\
Does the behaviour appear on standard tasks? & Lorenz-$x$, Mackey--Glass, delayed memory, and NARMA10 & Mean safe-region gain $0.00463$ [$0.00016$, $0.00911$] \\
\bottomrule
\end{tabularx}
\end{table}
...
The first experiment asks whether validation identifies a useful neighborhood of temperatures around the selected default. Table~\ref{tab:validation_summary} summarizes the numerical results and Fig.~\ref{fig:m19_validation} shows the transfer and matched-control comparisons. On held-out test data, the safe region contained at least one near-optimal temperature in $78.1\%$ of runs and contained the exact best temperature on the tested grid in $57.3\%$ of runs. The best temperature available inside the safe region improved over the fixed validation default by a mean of $0.004702$ NRMSE, with confidence interval $[0.003565,0.005786]$.

Because NRMSE is normalized by the target standard deviation, this mean gain is equivalent to reducing RMSE by about $0.47\%$ of one target standard deviation. The best temperature over the entire grid produced a larger gain of $0.030254$, or about $3.03\%$ of one target standard deviation. The safe region is deliberately conservative: it captures the validation-supported part of the improvement available over the full path.

The next experiment measures whether the validation-selected region contains useful test temperatures more often than alternative intervals of exactly the same width. The core transfer experiment and the matched-control experiment use independently generated run panels and have different denominators. They answer related but distinct questions. The first estimates how often the validation-safe region transfers successfully. The second compares the validation-selected region with alternative intervals of exactly the same width. In that matched-control experiment, the validation-selected region contained a near-optimal test temperature in $79.2\%$ of runs, compared with $38.5\%$ for same-width control intervals. Exact-best containment was $63.5\%$ for the validation-selected region and $15.6\%$ for the controls. The mean gain advantage of the validation-selected region over the control intervals was $0.02739$, with confidence interval $[0.01870,0.03710]$. A separate movement comparison sampled candidate changes inside and outside the safe region. Moving to a candidate inside the region had a mean gain advantage of $0.03209$ over movement outside it, with confidence interval $[0.02342,0.04143]$. Figure~\ref{fig:m19_validation} visualizes both comparisons. Together, these results show that validation identifies a non-arbitrary part of the allocation-temperature path.

\begin{figure}[t]
\centering
\begin{subfigure}{0.48\textwidth}
\includegraphics[width=\textwidth]{figures/fig2_core_validation_summary.png}
\caption{Gain available inside the validation-safe region.}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
\includegraphics[width=\textwidth]{figures/fig3_safe_region_controls.png}
...
This expectation is supported, although the relationships are moderate rather than deterministic. Across test windows, prediction dispersion was positively correlated with the improvement available from choosing the best safe temperature ($r=0.412$). The low/high endpoint spread showed a similar correlation ($r=0.405$). Test windows in the highest quartile of prediction dispersion had, on average, $0.01689$ more available gain than windows in the lowest quartile, with confidence interval $[0.01227,0.02196]$. The corresponding difference for low/high anchor spread was $0.01702$, with confidence interval $[0.01243,0.02207]$.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\textwidth]{figures/fig4_path_sensitivity.png}
...
The standard-task study tests how the same calibration behaves across Lorenz-$x$, Mackey--Glass, delayed memory, and NARMA10. Figure~\ref{fig:standard_boundary} shows the task-level pattern. It includes Lorenz-$x$, Mackey--Glass, delayed memory, and NARMA10. Across the panel, the safe region contained a near-optimal test temperature in $96.9\%$ of runs and the exact best temperature on the tested grid in $78.1\%$. Mean safe-region gain was positive on all four tasks and averaged $0.004634$, with confidence interval $[0.000160,0.009109]$. This corresponds to about $0.46\%$ of one target standard deviation in RMSE.

The task-level pattern is more informative than the average. Lorenz-$x$ and Mackey--Glass have broad performance plateaus: the validation-selected default is already close to the best temperature, so little improvement remains inside or outside the safe region. Delayed memory and NARMA10 are more temperature-sensitive and leave more room for movement along the full path. The path-sensitivity indicators are therefore most informative when allocation temperature produces meaningful variation in predictions and performance. On a broad performance plateau, little improvement is available for any indicator to reveal. The $78.1\%$ exact-containment value in this separate four-task study happens to equal the near-optimal containment percentage of the earlier transfer experiment; the two values come from different panels and measure different events.

\begin{figure}[t]
\centering
\includegraphics[width=0.70\textwidth]{figures/fig5_standard_task_boundary.png}
\caption{The value of allocation-temperature movement depends on the task. Lorenz-$x$ and Mackey--Glass show broad plateaus with little additional improvement available over the tested temperature range, whereas delayed memory and NARMA10 show stronger sensitivity.}
...
\toprule
Path & Effective-support range & Spectral-radius range & Full-test best-safe gain & Safe-control gain & Interpretation \\
\midrule
Temperature & $55.19$ & $0.046$ & $0.00230$ & $+0.02303$ & continuous concentration path \\
Gain & $0.00$ & $1.438$ & $0.00241$ & $+0.03173$ & amplification/spectral-radius path \\
Leak-rate & $0.00$ & -- & $0.00422$ & $+0.06486$ & update-timescale path \\
Sparsity & $58.00$ & task-dependent & $0.00015$ & $+0.04160$ & hard top-$k$ support path \\
\bottomrule
\end{tabular}}
\end{table}

The comparison supports three conclusions. First, allocation temperature changes the concentration of recurrent weight mass in a way that gain and leak rate do not. Its mean effective-support range is $55.19$ equivalent inputs, while gain and leak-rate paths leave this concentration measure essentially unchanged. At the same time, the spectral-radius range along the temperature path is only $0.046$, whereas the gain path changes spectral radius by $1.438$ while leaving effective support fixed. These complementary signatures distinguish allocation temperature from ordinary amplification. Second, the temperature safe region remains enriched relative to same-width controls: its mean safe-minus-control gain is $+0.02303$ with a positive confidence lower bound. Third, under the common full-test best-safe metric, gain and leak rate provide larger within-safe-region improvement in the synthetic comparison. Allocation temperature should therefore be treated as an additional structural coordinate, not as a replacement for existing controls.

Hard sparsity is the closest structural comparison because it also changes how many inputs matter. The difference is that top-$k$ sparsity sets excluded edges exactly to zero, whereas allocation temperature continuously redistributes weight mass across the allowed edges. In this sense, allocation temperature provides a continuous concentration control that can move between highly focused and diffuse recurrent rows without changing the hard connectivity pattern. The hard-sparsity path has a similar effective-support range ($58.00$) but smaller full-test safe-region improvement ($0.00015$) than allocation temperature ($0.00230$) in this comparison.

\begin{figure}[t]
\centering
\begin{subfigure}{0.48\textwidth}
\includegraphics[width=\textwidth]{figures/fig6_structural_ablation.png}
\caption{Support range by path.}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
\includegraphics[width=\textwidth]{figures/fig7_operational_ablation.png}
\caption{Improvement available inside each validation-safe region.}
\end{subfigure}
\caption{Path ablations. Allocation temperature and sparsity change recurrent support; gain and leak-rate do not. Panel (b) reports full-test best-safe NRMSE gain under a common metric across controls. Operationally, allocation temperature is useful and structurally distinct but not dominant over all one-dimensional paths.}
...
\begin{table}[t]
\centering
\caption{Retained historical real-world sensor benchmark summary. Appliances Energy yields a null safe-region-versus-shuffled result, whereas Air Quality gives a positive safe-region result. The current reconstruction notebooks recover the documented structural path but not these exact operational point estimates; fresh reconstruction outputs are reported separately in the repository.}
\label{tab:real_world}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllllll}
\toprule
Dataset & Rows/features & Target & Temp. support range & Temp. full-test best-safe gain & Temp. safe $-$ control & Temp. path indicators \\
\midrule
Appliances Energy & $19729/32$ & log future appliances, $h=6$ & $45.62$ & $0.00014$ & $-0.01199$ [$-0.05331$, $0.01532$] & $0.510$ / $0.522$ \\
Air Quality & $9356/19$ & future C6H6, $h=1$ & $45.55$ & $0.01635$ & $+0.05312$ [$0.00695$, $0.09534$] & $0.727$ / $0.646$ \\
\bottomrule
\end{tabular}}
\end{table}

The retained historical summaries provide complementary evidence for how the allocation-temperature axis behaved on measured sensor data. On Appliances Energy, allocation temperature retains its structural signature and its two path-sensitivity indicators remain informative, but its full-test best-safe gain is only $0.00014$, while the safe-region-minus-shuffled point estimate is negative ($-0.01199$) and its confidence interval crosses zero ($[-0.05331,0.01532]$). This is a null result and does not support safe-region superiority on that dataset; leak-rate also provides stronger available safe-region improvement there. On Air Quality, the retained result is stronger: the temperature safe region outperforms same-width controls, prediction dispersion and low/high endpoint spread correlate strongly with the improvement available inside the safe region, and allocation temperature gives the largest within-safe-region improvement among the four tested paths. Because the original June 2026 M20b/M20c2 notebook binaries could not be recovered, Table~\ref{tab:real_world} reports retained historical summaries. The repaired public-data reconstructions preserve the documented dataset dimensions and structural temperature path but do not numerically recover these operational point estimates; fresh outputs are reported separately under \path{results/reproduced/}.
...
The validation safe region is intentionally conservative. At the default relative tolerance $\epsilon_{\mathrm{rel}}=0.10$, using each study's native absolute floor, the temperature safe region occupies only a small fraction of the 13-point candidate grid: mean widths are $3.23$ candidate points in the synthetic path-comparison study, $1.75$ candidate points on Appliances Energy, and $2.50$ candidate points on Air Quality (Table~\ref{tab:m21_safe_width}). Relative to all same-width intervals on the same grid, near-optimal containment is enriched by factors of $3.12$, $4.25$, and $3.63$, respectively. These factors should be interpreted as finite-grid empirical quantities, not as grid-resolution-invariant constants. Under the reported grids, the enrichment over same-width controls shows that the safe region identifies a more informative part of the candidate path than interval width alone would provide. For the fully regenerated synthetic path-comparison audit, sensitivity checks over $\epsilon_{\mathrm{rel}}\in\{0.05,0.10,0.15,0.20\}$ preserve the qualitative conclusion: increasing tolerance widens the safe component while same-width interval comparisons remain the appropriate null model. The real-data rows below report the retained historical native $\epsilon_{\mathrm{rel}}=0.10$ summaries.

\begin{table}[t]
\centering
\caption{Safe-region width at $\epsilon_{\mathrm{rel}}=0.10$ using each study's native absolute tolerance. The validation-selected regions occupy a small fraction of the tested grid and contain near-optimal test points more often than same-width control intervals. Full-test best-safe gain is reported with the same definition in all rows.}
\label{tab:m21_safe_width}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llllll}
\toprule
Study & $\epsilon_{\mathrm{abs}}$ & Mean safe width & Width fraction & Near-optimal containment ratio & Full-test best-safe gain \\
\midrule
Synthetic path comparison & $0.005$ & $3.23/13$ & $24.8\%$ & $3.12\times$ & $0.00230$ \\
Appliances Energy & $0.002$ & $1.75/13$ & $13.5\%$ & $4.25\times$ & $0.00014$ \\
Air Quality & $0.002$ & $2.50/13$ & $19.2\%$ & $3.63\times$ & $0.01635$ \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{figures/fig8_robustness_containment.png}
...
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}Xccc>{\centering\arraybackslash}p{0.16\textwidth}}
\toprule
Held-out shift & Pre-deployment fixed & Sparse ensemble & Relative gain [95\% CI] & Windows harmed \\
\midrule
...
Table~\ref{tab:operational_interpretation} summarizes conservative actions supported by the evidence. Low prediction variation favors retaining the validation-selected default because the accepted temperatures produce similar outputs. High variation is a monitoring signal, not an automatic direction of movement. If a shift type has been validated previously, a pre-deployment ensemble may provide a useful fallback. Information-removing shifts call for missing-data handling, abstention, fallback sensing, recalibration, or delayed-label intervention.
...
The retained historical real-world summaries are consistent with the same interpretation: Appliances Energy gives a null safe-region-versus-control result, whereas Air Quality gives a positive one. Because current public-data reconstructions do not numerically recover those operational point estimates, we treat this panel as supporting historical evidence rather than independently reproduced validation.
...
Allocation temperature adds a distinct structural configuration dimension to reservoir computing. It continuously changes how recurrent weight mass is distributed across incoming connections, while a separate normalization keeps bulk recurrent scale comparable. Row entropy and effective support increase monotonically with temperature, giving the parameter a direct structural interpretation that differs from recurrent gain, leak rate, and hard sparsity.

Validation turns this structural axis into an empirical operating path. The validation-safe temperature region is enriched for useful held-out temperatures relative to same-width control intervals, and prediction variation across that region indicates when temperature choice matters more on a test window. These conclusions persist under a denser temperature grid and a larger reservoir, although the direct NRMSE improvement available inside the safe region is modest and fixed binary thresholds are less robust than the continuous indicator relationship.

The final distribution-shift study shows why the structural dimension remains interesting beyond those modest within-region gains. When temperature and leak rate are considered jointly, target labels reveal much larger structural improvement than can be identified automatically from unlabeled target data. A sparse ensemble fitted before deployment recovers part of that opportunity, but its reliability depends on the type of shift and breaks down under block missingness. Allocation temperature therefore provides both a new reservoir design coordinate and a controlled way to study when recurrent allocation matters, what can be inferred without labels, and where additional adaptation mechanisms are still required.

\section*{Code and Data Availability}

The public reproducibility package is available in the \href{https://github.com/ildefons/temperature-calibrated-reservoirs}{project repository}. It contains the manuscript source, frozen manuscript tables and figures, environment specifications, computational notebooks, and verification checks. Appendix~\ref{app:repro_map} maps each main-text result to the corresponding computational artifact. The repository distinguishes original executed analyses retained for M23, SRP04b, and SRP05 from explicitly labelled publication-reconstruction notebooks for M19a--M21, whose original June 2026 notebook binaries could not be recovered. Historical manuscript values and fresh reconstructed outputs are kept separate, with the distinction documented in \path{PROVENANCE.md}. Fresh M20b/M20c2 executions recover the documented structural path but not the retained historical operational point estimates. The public datasets are the UCI Appliances Energy Prediction and UCI Air Quality datasets; they are not redistributed in the repository, and \path{data/README.md} provides the expected filenames and source information.
...
The binary threshold analyses are retained for completeness but are not primary claims. They depend on the base positive-gain rate, the top-quartile threshold, and the number of distinct operating points inside each safe region. In the original path-comparison and real-data studies, high-indicator windows showed descriptive enrichment, as summarized in Table~\ref{tab:secondary_threshold_original}. Under the grid-density and reservoir-size interventions, however, the same binary precision ratio is near or below one and its confidence intervals cross one. The primary indicator result is therefore the continuous association between prediction variation and available safe-region improvement.

\begin{table}[h]
\centering
\caption{Secondary top-quartile threshold analysis for the original path-comparison and real-data studies. High-indicator windows are the top quartile of safe-region dispersion or low/high endpoint spread. These values are descriptive for the corresponding study settings.}
...
\midrule
Synthetic path comparison & $0.40$ & $0.65$--$0.67$ & $1.62$--$1.68\times$ & $\approx0.025$ \\
Appliances Energy & $0.275$ & $0.50$--$0.55$ & $1.82$--$2.00\times$ & $\approx0.0155$ \\
Air Quality & $0.433$ & $0.867$ & $2.00\times$ & $0.087$--$0.104$ \\
\bottomrule
\end{tabular}}
\end{table}
...
Table~\ref{tab:repro_map} provides the file-level mapping from manuscript results to the computational record. The public repository contains the publication notebooks, frozen manuscript tables, fresh reconstructed outputs where applicable, figure assets, environment specifications, and verification checks. Repository provenance is explicit: M23, SRP04b, and SRP05 retain their original executed analyses, whereas M19a--M21 are publication reconstructions because the original June 2026 notebook binaries could not be recovered.
...
Section~\ref{sec:real_world}, Table~\ref{tab:real_world} & Retained historical Appliances Energy and UCI Air Quality summaries & M20b, M20c2 (public-data reconstructions) \\
Section~\ref{sec:additional_validation}, Table~\ref{tab:m21_safe_width}, Appendix~\ref{app:binary_threshold} & Safe-region width, tolerance, spectral-radius comparison, secondary threshold analysis & M21 \\
...
During the preparation of this work, the author used OpenAI ChatGPT to assist with language refinement, manuscript organization, clarification of the theoretical presentation, and the drafting and debugging of computational code. The author drafted the theoretical arguments and manuscript content, critically evaluated, revised, and verified all theory, computational code, experimental results, and manuscript material, made all final scientific decisions, and takes full responsibility for the published work.
