...
\end{equation}
Unless otherwise stated, $\epsilon_{\mathrm{abs}}=0.005$ and $\epsilon_{\mathrm{rel}}=0.10$. The relative term scales the tolerance with the observed validation range, while the absolute floor avoids an unrealistically narrow threshold on nearly flat validation curves. The same numerical tolerance is used retrospectively when identifying near-optimal test candidates. In the tolerance-sensitivity analysis, $\epsilon_{\mathrm{abs}}$ remains fixed and $\epsilon_{\mathrm{rel}}$ is varied.
...
\end{subfigure}
\caption{Path ablations. Allocation temperature and sparsity change recurrent support; gain and leak-rate do not. Operationally, allocation temperature is useful and structurally distinct but not dominant over all one-dimensional paths.}
...
\toprule
Dataset & Rows/features & Target & Temp. support range & Temp. best-safe gain & Temp. safe $-$ control & Temp. path indicators \\
\midrule
Appliances Energy & $19729/32$ & log future appliances, $h=6$ & $45.62$ & $0.00341$ & $-0.01199$ [$-0.05331$, $0.01532$] & $0.510$ / $0.522$ \\
Air Quality & $9356/19$ & future C6H6, $h=1$ & $45.55$ & $0.01635$ & $+0.05312$ [$0.00695$, $0.09534$] & $0.727$ / $0.646$ \\
\bottomrule
\end{tabular}}
\end{table}

The two datasets provide complementary evidence for how the allocation-temperature axis behaves on measured sensor data. On Appliances Energy, allocation temperature retains its structural signature and its two path-sensitivity indicators remain informative, but the safe-region-minus-shuffled point estimate is negative ($-0.01199$) and its confidence interval crosses zero ($[-0.05331,0.01532]$). This is a null result and does not support safe-region superiority on that dataset; leak-rate also provides stronger available safe-region improvement there. On Air Quality, the result is stronger: the temperature safe region outperforms same-width controls, prediction dispersion and low/high endpoint spread correlate strongly with the improvement available inside the safe region, and allocation temperature gives the largest within-safe-region improvement among the four tested paths. Together, the two datasets show that the structural behavior transfers to measured sensor data, while the size of the operational benefit remains dataset-dependent.
...
The validation safe region is intentionally conservative. At the default relative tolerance $\epsilon_{\mathrm{rel}}=0.10$, the temperature safe region occupies only a small fraction of the 13-point candidate grid: mean widths are $2.40$ candidate points in the synthetic path-comparison study, $1.75$ candidate points on Appliances Energy, and $2.50$ candidate points on Air Quality (Table~\ref{tab:m21_safe_width}). Relative to all same-width intervals on the same grid, near-optimal containment is enriched by factors of $3.73$, $4.25$, and $3.63$, respectively. These factors should be interpreted as finite-grid empirical quantities, not as grid-resolution-invariant constants. Under the reported grids, the enrichment over same-width controls shows that the safe region identifies a more informative part of the candidate path than interval width alone would provide. Sensitivity checks over $\epsilon_{\mathrm{rel}}\in\{0.05,0.10,0.15,0.20\}$ preserve the qualitative conclusion: increasing tolerance widens the safe component and increases containment, while the same-width interval comparison remains the appropriate null model.

\begin{table}[t]
\centering
\caption{Safe-region width at $\epsilon_{\mathrm{rel}}=0.10$. The validation-selected regions occupy a small fraction of the tested grid and contain near-optimal test points more often than same-width control intervals.}
\label{tab:m21_safe_width}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
\toprule
Study & Mean safe width & Width fraction & Near-optimal containment ratio & Safe gain \\
\midrule
Synthetic path comparison & $2.40/13$ & $18.5\%$ & $3.73\times$ & $0.00504$ \\
Appliances Energy & $1.75/13$ & $13.5\%$ & $4.25\times$ & $0.00341$ \\
Air Quality & $2.50/13$ & $19.2\%$ & $3.63\times$ & $0.01635$ \\
\bottomrule
\end{tabular}}
\end{table}
...
The computational record includes the experiment notebooks, generated tables, figure scripts, environment information, exact random seeds, and manuscript source. Appendix~\ref{app:repro_map} maps each main-text result to the corresponding computational artifact. The public datasets are the UCI Appliances Energy Prediction and UCI Air Quality datasets. A public archival repository URL will be inserted before journal submission; until that release is created, the complete reproducibility package is available from the author on request.
...
Table~\ref{tab:repro_map} provides the file-level mapping from manuscript results to the computational record. The repository will contain the executed notebooks, generated CSV tables, figure-generation scripts, environment specification, and exact seeds used for the reported results.
...
\noindent\textbf{Repository:} public archival URL to be inserted before journal submission.
