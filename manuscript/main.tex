...
Validation turns this structural axis into an empirical operating path. In the controlled validation panels, the validation-safe temperature region is enriched for useful held-out temperatures relative to same-width control intervals, and prediction variation across that region indicates when temperature choice matters more on a test window. The real-data panel shows that this localization can transfer positively, as on Air Quality, but can also fail to beat matched intervals, as on Appliances Energy. These conclusions persist under a denser temperature grid and a larger reservoir, although the direct NRMSE improvement available inside the safe region is modest and fixed binary thresholds are less robust than the continuous indicator relationship.
