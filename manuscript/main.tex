...
\begin{table}[t]
\centering
\caption{Real-world sensor benchmark summary. Appliances Energy yields a null safe-region-versus-shuffled result, whereas Air Quality gives a positive safe-region result. Both preserve the structural distinction among the tested paths, while the operational benefit depends on the dataset.}
...
The two datasets provide complementary evidence for how the allocation-temperature axis behaves on measured sensor data. On Appliances Energy, allocation temperature retains its structural signature and its two path-sensitivity indicators remain informative, but its full-test best-safe gain is only $0.00014$, while the safe-region-minus-shuffled point estimate is negative ($-0.01199$) and its confidence interval crosses zero ($[-0.05331,0.01532]$). This is a null result and does not support safe-region superiority on that dataset; leak-rate also provides stronger available safe-region improvement there. On Air Quality, the result is stronger: the temperature safe region outperforms same-width controls, prediction dispersion and low/high endpoint spread correlate strongly with the improvement available inside the safe region, and allocation temperature gives the largest within-safe-region improvement among the four tested paths. Together, the two datasets show that the structural behavior transfers to measured sensor data, while the size of the operational benefit remains dataset-dependent.
...
The real-world benchmarks reinforce the same interpretation. On Appliances Energy, allocation temperature retains its structural meaning and the path-sensitivity indicators remain informative, but the safe-region-versus-control comparison is null. On Air Quality, the validation-safe temperature region outperforms same-width controls and allocation temperature gives the largest within-safe-region improvement among the tested paths. The practical value of allocation temperature is therefore task-dependent, while its role as a distinct structural coordinate is consistent across the controlled comparisons.
...
Section~\ref{sec:real_world}, Table~\ref{tab:real_world} & Appliances Energy and UCI Air Quality & M20b, M20c2 \\
